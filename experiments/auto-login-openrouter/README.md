# Experimental auto login via OpenRouter

This folder contains a standalone test flow for GateWa.rs that:

- logs in if needed
- solves the PIN captcha with Gemini 3.1 Pro through OpenRouter
...
- `openrouter_pin_solver.py`: OpenRouter request/response handling for the PIN image
...
```bash
uv run python experiments/auto-login-openrouter/login_test.py
```

Optional flags:

```bash
uv run python experiments/auto-login-openrouter/login_test.py --headless
uv run python experiments/auto-login-openrouter/login_test.py --model google/gemini-3.1-pro-preview
```
...
Set the OpenRouter key like this:
...
```text
experiments/auto-login-openrouter/profile/
```

## OpenRouter behavior

The solver uses OpenRouter's raw chat completions API with:

- model: `google/gemini-3.1-pro-preview`
- provider order: `google-vertex`, then `google-ai-studio`
- fallback enabled inside OpenRouter
- reasoning effort: `low`
- reasoning output excluded from the response body
- `max_tokens` capped at `1024`
- strict JSON schema output for `{"numbers": [1, 2, 3]}`

The local retry loop still retries whole OpenRouter requests for retryable transport
errors and HTTP `429`, `500`, `502`, `503`, and `504` responses.
...
ExecStart=/snap/bin/uv run python experiments/auto-login-openrouter/login_test.py --headless
```
